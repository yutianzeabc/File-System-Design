...
int my_write(int fd,int mode){
    ...
    if(mode==0){ //截断写
        int init_blocknum=*(begin_addr+sizeof(int));
        char *init_addr=(char*)virtualDisk+BLOCK_SIZE*init_blocknum;
        ...
        for(i=0;i<=num_block_allocated;i++){
            int logic_num=*begin_addr;
            printf("logic_block[%d]:%d ;",i,logic_num);
            int physics_blocknum=*(begin_addr+sizeof(int));
        ...
        if(last_pointer>0){
            int init_blocknum=*(begin_addr+(i*2+1)*sizeof(int));
            init_addr=(char*)virtualDisk+BLOCK_SIZE*init_blocknum;
            ...
            if (i!=0)//注意：原始文件并非刚打开的空文件（逻辑盘块0肯定是以及分配的）
            {
                ...
                *(begin_addr+(2*cnt+1)*sizeof(int))=new_blocknum;//写到文件索引表 第二列
                cnt=cnt+1;
                char *init_addr=(char*)virtualDisk+BLOCK_SIZE*new_blocknum;
                cursor_addr=init_addr;
            }
        }else{
